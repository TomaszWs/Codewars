...
#
# One of the simplest, yet most important factors is compensation. As
# developers we know how much money we need to support our lifestyle, so we
...
# which takes a candidate and a job, which will return a Boolean indicating
...
# }
#
# If either the candidate's minimum salary or the job's maximum salary is not
# present, throw an error.
#
# For a valid match, the candidate's minimum salary must be less than or equal
# to the job's maximum salary. However, let's also include 10% wiggle room
# (deducted from the candidate's minimum salary) in case the candidate is a
# rockstar who enjoys programming on Codewars in their spare time. The company
