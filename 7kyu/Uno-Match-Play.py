...
# Task
#
# Write a function that takes in two arguments: (1) a player's current
# hand and (2) the current face-up card on the table. The function
# will return True if the player can make a play, or False if the
# player has to draw from the deck.
#
...
def can_play(hand, face_up):
    if not hand:
        return False
    color, num = face_up.split()
    return any(
        i.split()[0] == color or i.split()[1] == num for i in hand)
...
def can_play(hand, face_up):
    return any(w in face_up for c in hand for w in c.split())


def can_play(hand, face_up):
    f, n = face_up.split()
    return any(c == f or v == n for c, v in (card.split() for card in hand))
